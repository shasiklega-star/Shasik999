-- Безопасный запуск с задержкой, чтобы Delta успела прогрузить окружение
task.wait(0.5)
...
local success, err = pcall(function()
    ...
    -- ==========================================
    -- БЕЗОПАСНАЯ КРУГЛАЯ КНОПКА (БЕЗ ССЫЛОК НА КАРТИНКИ)
    -- ==========================================
    ...
    ToggleButton.Text = "★" -- Простой текстовый символ вместо проблемного Image ID
    ...
    RunService.Heartbeat:Connect(function(deltaTime) -- Использование Heartbeat вместо RenderStepped для стабильности на Delta
        if not MainFrame.Visible then return end
        
        if #snowflakes < maxSnowflakes and math.random() > 0.85 then
            createSnowflake()
        end
    ...
    print("[good]: Интерфейс успешно запущен.")
end)

-- Вывод ошибки в консоль Roblox (F9), если Delta споткнется на коде
if not success then
    warn("[good] Ошибка выполнения скрипта: " .. tostring(err))
end
