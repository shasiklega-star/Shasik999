-- ============================================
-- SHASIK PRISON RP ULTIMATE EDITION
-- ПОЛНЫЙ СКРИПТ | 100% РАБОТАЕТ | ОПТИМИЗИРОВАН
-- ============================================
...
function playClickTab()
    if not soundEnabled then return end
    pcall(function()
        local s = Instance.new("Sound", workspace)
        s.SoundId = "rbxassetid://9120381962"
        s.Volume = 0.3
        s.PlayOnRemove = true
        s:Play()
        task.wait(0.2)
...
frame.BackgroundColor3 = settings.theme == "dark" and Color3.fromRGB(4, 4, 18) or Color3.fromRGB(240, 240, 255)
...
catFrame.Size = UDim2.new(0.92, 0, 0, 38)
catFrame.Position = UDim2.new(0.04, 0, 0, 85)
...
function updateContent()
    ...
    elseif currentCategory == 5 then
        ...
        resetBtn.MouseButton1Click:Connect(function()
            ...
            settings.theme = "dark"
            settings.iconSize = 55
            settings.menuAlpha = 5
...
task.spawn(function()
    local targetSize = UDim2.new(0, 500, 0, 540)
    local targetPos = UDim2.new(0.5, -250, 0.5, -270)
    local steps = 35
    for i = 1, steps do
        local t = i / steps
        local ease = t * t * (3 - 2 * t)
        frame.Size = UDim2.new(0, 500 * ease, 0, 540 * ease)
        frame.Position = UDim2.new(0.5, -250 * ease, 0.5, -270 * ease)
...
-- ============================================
-- ПЛАВНОЕ ОТКРЫТИЕ/ЗАКРЫТИЕ ПО ИКОНКЕ
-- ============================================
...
function toggleMenu()
    ...
    if menuVisible then
        ...
        for i = 1, 30 do
            local t = i / 30
            local ease = t * t * (3 - 2 * t)
            frame.Size = UDim2.new(0, 500 * (1 - ease), 0, 540 * (1 - ease))
            frame.Position = UDim2.new(0.5, -250 * (1 - ease), 0.5, -270 * (1 - ease))
...
iconBtn.BackgroundColor3 = Color3.fromRGB(4, 4, 18)
...
local shadow = Instance.new("Frame", iconBtn)
...
spawn(function()
    while true do
        ...
        shadow.BackgroundTransparency = 0.5 + math.sin(t + 1) * 0.2
        task.wait(0.02)
    end
end)
...
print("✅ SHASIK PRISON RP ULTIMATE EDITION ЗАГРУЖЕН!")
